...
from latent_action_model import LatentActionVQVAE, load_latent_action_model, extend_codebook, mask_frozen_embeddings
import torchvision.transforms as T


frame_transform = T.Compose([
    # T.ToTensor(),                     # Converts from HWC uint8 in [0, 255] to CHW float32 in [0.0, 1.0]
    
    T.ConvertImageDtype(torch.float16)  # Optional: convert to float16 if your model uses it
])
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     raise IndexError("Index out of range for video dataset")
        frame_t = self.all_frames[idx]
        frame_t1 = self.all_frames[idx + 1]
        if self.transform:
            frame_t = self.transform(frame_t)
            frame_t1 = self.transform(frame_t1)
    ...
    def extract_frames(self, video_path):
        ...
        while True:
            ...
            frames.append(torch.tensor(frame).permute(2, 0, 1))  # Convert to CxHxW
        cap.release()
        return frames
...
def save_checkpoint(state, is_best, checkpoint_dir, step):
    ...
    if is_best:
        torch.save(state, os.path.join(checkpoint_dir, "best_new.pt"))
...
def train(args):
    ...
    dataset = VideoDataset(args.data_dir, transform=frame_transform)  # No transform for raw frames
    train_set, val_set, test_set = torch.utils.data.random_split(dataset, [800, 100, 99])
    ...
    def get_stats(dataset, name):
        ...
        print(f"{name}: {n_pairs} pairs, frame shape: {shape}")
    print("--- Data Statistics ---")
    ...
    # Model
    model = LatentActionVQVAE(freeze_old=True)
    model = model.to(device)
    
    #load the previous best model
    model,_ = load_latent_action_model("checkpoints/latent_action/best.pt", device=device)
    ...
    extend_codebook(model=model, new_embeddings=32)
    
    model.to(device)  # Ensure model is on the correct device after compilation
    ...
    while global_step < args.max_iters:
        ...
        with torch.amp.autocast(device.type, enabled=(scaler is not None)):
            # print(frame_t.shape, frame_tp1.shape)
            # frame_t = frame_t.permute(0, 1, 3, 2)  # (B, C, H, W)
            # frame_tp1 = frame_tp1.permute(0, 1, 3, 2)
            recon, indices, commit_loss, codebook_loss = model(frame_t, frame_tp1)
            ...
            entropy, hist = codebook_entropy(indices, 256 + 32)
        ...
        if scaler is not None:
            ...
            mask_frozen_embeddings(model.vq.embeddings, old_count=256)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='videos', help='Directory with episode folders')
